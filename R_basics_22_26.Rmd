---
title: "**R basics chapter 2.2 ~ 2.6**"
output:
  html_document:
    df_print: paged
---
...
<center> <h3> ${-b + \sqrt{b^2-4ac} \over 2a}$ and ${-b - \sqrt{b^2-4ac} \over 2a}$ </h3> </center>
...
By creating and saving a script with the code above, we would not need to retype everything each time and, instead, simply change the variable names. Try writing the script above into an editor and notice how easy it is to cahnge the variables and receive an answer.

### **2.2.8 Commenting your code**

If a line of R code starts with the symbol `#`, it is not evaluted. We can use this to write remiders of why we wrote particular code. For example, in the script above we could add:

```{r}
## code to compute solution to quadratic equation of the  the form ax^2 + bx + c
...
2\. Now use the same formula to compute the summ of the integers from 1 through 1,000.
...
3\. Look at the result of typing the follwing code into R:
```{r}
n <- 1000
x <- seq(1,n)
sum(x)
...
c. `seq` creates a random list and `sum` computes the sum of 1 throgh 1,000.  
...
To work efficeintly in R, it is important to learn the different types of variables and what we can do with these.

### **2.4.1 Data frames**

Up to now, the variables we have defined are just one number. This is not very useful for storing data. The most common way of storing a dataset in R is in a _data frame_. Conceptually, we can think of a data frame as a table with rows representing observations and the different variables reported for each observation defining the columns. Data frames are particularly useful for datasets because we can combine differnet data types into one object.
...
This tell us much more about the object. We see that the table has 51 rows (50 states plus DC) and five variables. We can show the first six lines using the function `head`:
...
For our ananlysis, we will need to access the different variables represented by columns included in this data frame. To do this, we use the accessor operator `$` in the following way:
...
**Tip**: R comes with a very nice auto-complete functionality that saves us the trouble of typing out all the names. Try typing `murder$p` then hitting the _tab_ key on your keyboard. This functionality and many other useful auto-complete features are available when working in RStudio.
...
To store character strings, vectors can also be of class _character_. For example, the state names are charaters:
...
**Advanced**: Mathematically, the values in `pop` are integers and there is an integer class in R. However, by default, numbers are assigned class numeric even when they are round integers. For example, `class(1)` returns numeric. You can turn them into class integer with the `as.integer()` function or by adding an `L` liek this: `1L`. Note the class by typing: `class(1L)`
...
Note that the levels have an order that is different from the order of appearance in the facotr object. The default in R is for the levels to follow alphabetical order. However, often we want the levels to follow a different order. You can specify an order through the `levels` argument when creating the factor with the `factor` function. For example, in the murders dataset regions are ordered from east to west. The function `reorder` lets us change the order of the levels of a factor variable based on a summary computed on a numeric vector. We will demonstrate this with a simple example, and will see more advanced ones in the Data Visualization part of the book.
...
If a list does not have names, you cannot extract the elements with `$`, but you can still use the brackets method and instead of providing the variable name, you provide the lsit index, like this
...
You can access specific entries in a matix using square brackets (`[`). If you want the second row, third column, you use: 
...
We can convery matices into data frames using the function  
`as.data.frame`:
...
```{r}
table(reorder(murders$region, murders$total, FUN = sum))
```
...
The first argument defines the start, and the second defines the end which is included. The default is to go up in increments of 1, but a thrid argument lets us tell it how much to jump by:

```{r}
seq(1,10,2)
```

If we want consecutive integers, we can use the follwing shorthand:

```{r}
1:10
```

When we use these functions, R produces integers, not numerics, because they are typically used to index somthing:
...
If elements have names, we can also access the entries using these names. Below are two examples.

...
```{r}
library(dslabs)
head(murders <- murders)
```

Each row represent a state with each of the five columns providing a different variable realted to these states: name, abbreviation, region, population, and total murders.

To see how the same information can be provided in different formats, consider the following example: 

```{r, echo = FALSE}
country <- factor(c("Germany", "South Korea","Germany", "South Korea", "Germany", "South Korea"))
year <- factor(c(1960, 1960, 1961, 1961, 1962, 1962))
fertility <- c(2.41, 6.16, 2.44, 5.99, 2.47, 5.79)

data.frame(index = 1:6, country, year, fertility)
```

This tidy dataset provides fertility rates for two countries across the years. This is tidy dataset because each row presents one observation with three variables being country, year, and fertility rate. However, this dataset originally came in another format and was reshaped for the **dslabs** package. Originally, the data was in the following format:

```{r,echo=FALSE}
country_2 <- c("Germany", "South Korea")
yr_1960 <- c(2.41, 6.16)
yr_1961 <- c(2.44, 5.99)
yr_1962 <- c(2.47, 5.79)

data.frame(index = 1:2, country_2, yr_1960, yr_1961, yr_1962)
```
...
```{r}
# All of the above are tidy data.
```

## **4.3 Manipulating data frames**

The **dplyr** package from the **tidyverse** introduces that perform some of the most common operations when working with data frames and uses names for these functions that are relatively easy to remember. For instance, to change the data table by adding a new column, we use `mutate`. To filter the data table to a subset of rows, we use `filter`. Finally, to subset the data by selecting specific columns, we use `select`.
...
Notic that here we used `total` and `population` inside the function, which are objects that are not defined in our workspace. But why don't we get an error?
...
Use the function `mutate` to add a murders column named `rate` with per 100,000 murders rate as in the example code above. Make sure you redefine `murders` as done in the example code above (murders <- [your code]) so we can keep using this variable.
...
2\. If `rank(x)` gives you the ranks of `x` from lowest to highest, `rank(-x)` gives you the ranks from highest to lowest. Use the function `mutate` to add column `rank` containing the rank, from highest to lowest murder rate. Make sure you redefine `murders` so we can keep using this variable.
...
4\. The **dplyr** function `filter` is used to choose specific rows of the data frame to keep. Unlike `select` which is for columns, `filter` is for orws. For example, you can show just the New York row like this:
...
Use `filter` to show the top 5 state with the highest murder rates. After we add murder rate and rank, do not change the murders dataset, just show the result. Remember that you can filter based on the `rank` column.
...
7\. Suppose you want to live in the Northeast or West **and** wantthe murder rate to be less than 1. We wnat to see the data for the states satisfying these options. Note that you can use logical operators with `filter`. Here is an example in which we filter to keep only small states in the Northeast region.
...
We wrote code above to show three variables (state, region, rate) for states that have murder rates below 0.71. To do this, we deifned the intermediate object `new_table`. In **dplyr** we can write code that looks more like a description of what we want to do without intermediate objects:
...
The above statement is equivalent to `log(sqrt(16))`
...
Therefore, when using the pipe with data frames and **dplyr**, we no longer need to specify the required first argument since the **dplyr** functions we have descirbed all take the data as the first argument. In the code we wrote:

```{r}
murders %>% select(state, region, rate) %>% filter(rate <= 0.71)
```

`murders` is the first argument of the `select` fuction, and the new data frame (formerly `new_table`) is the first argument of the `filter` function.
...
my_state <- murders %>% 
  ...
  filter(region %in% c("Northwest", "West") & rate < 1) %>% 
  select(state, rate, rank)

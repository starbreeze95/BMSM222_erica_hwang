...
This takes our original data talbe as input, filters it to keep only females, and then produces a new summarized table with just the average and the standard deviation of heights. We get to choose the names of the columns of the resulting table. For example, above we decided to use `average` and `standard_deviation`, but we could have used other names just the same.
...
This might be problematic if we want to use this result with functions that require a numeric value. Here we show a useful trick for aceessing values stored in data when using pipes: When a data object is piped that object and its columns can be accessing using the `pull` function. To understand what we mean take a look at this line of code:
...
A commone operation in data exploration is to first split data into groups and then compute summaries for each group. For example, we may want to compute the average and standard deviation for men's and women's heights separately. The `group_by` function helps us do this.
...
The result does not look very different from `heights`, except we see `Groups: sex [2]` when we print the object. Although not immediately obvious from its apperance, this is now a special data frame called a _grouped data frame_, and **dplyr** functions, in particular `summarize`, will behave differently when acting on this object. Conceptually, you can think of this table as many tables, with the same columns but not necessarily the same number of rows, stacked together in one object. When we summarize the data after grouping, this is what happens:
...
The `summarize` function applies the summarization to each gorup separately.
...
With `arrange` we get to decide which column to sort by. To see the states by murder rate, from lowest to highest, we arrange by `rate` insteadL
...
The ignore the `NA`s we can use the `na.rm` argument:
...
ref_avg <- NHANES %>% filter(AgeDecade == " 20-29") %>% 
  summarize(mean(BPSysAve, na.rm = TRUE)) %>% 
  pull()
...
```{r}
min_max <- NHANES %>% filter(AgeDecade == " 20-29") %>% 
  summarize(min(BPSysAve, na.rm = T), max(BPSysAve, na.rm = T))

min_max
```

4\. Compute the average and standard deviation for females, but for each age group separately rather than a selected decades as in question 1. Note that the age groups are defined by `AgeDecade`. Hint: rather than filtering by age and gender, filter by `Gender` and then use `group_by`.

```{r}
Female_BP <- NHANES %>% filter(Gender == "female") %>% 
  group_by(AgeDecade) %>% 
  summarize(average = mean(BPSysAve, na.rm = T), sd = sd(BPSysAve, na.rm = T))
...
```{r}
Male_BP <- NHANES %>% filter(Gender == "male") %>% 
  group_by(AgeDecade) %>% 
  summarize(average = mean(BPSysAve, na.rm = T), sd = sd(BPSysAve, na.rm = T))
...
```{r}
Group_BP <- NHANES %>% group_by(AgeDecade, Gender) %>% 
  summarize(average = mean(BPSysAve, na.rm = T), sd = sd(BPSysAve, na.rm = T))
...
```{r}
Male_40s_BP <- NHANES %>% filter(Gender == "male" & AgeDecade == " 40-49") %>% 
  group_by(Race1) %>%
  summarize(average = mean(BPSysAve, na.rm = T), sd = sd(BPSysAve, na.rm = T)) %>%
  arrange(average)
...
While data frame columns need to be vectors of numbers, strings, or logical values, tibbles can have more complex objects, such as lists of functions. Also, we can create tibbles with functions:
...
One of the advantages of using the pipe `%>%` is that we do not have to keep naming new objects as we manipuate the data frame. As a quick reminder, if we want to compute the median murder rate for states in the southern states, instead of typing:
...
If you do not use the parenthesis, then the function is not executed and istead `do` tries to return the function. This gives an error because `do` must always return a data frame. You can see the error by typing:
...
The common use for this function is to define categorical variables based on existing variables. For example, suppose we want to compare the murder rates in four groups of states: _New England_, _West coast_, _South_, and _other_. For each state, we need to ask if it is in New England, if it is not we ask, if it is in the West Coast, if not we ask if it is in the South, and if not we assign _other_. Here is how we use `case_when` to do this:
...
```{r}
murders_tibble <- as_tibble(murders <- murders)
...
df <- data.frame(n = n, s_n = map_df(n,func1), s_n_2 = s_n)
